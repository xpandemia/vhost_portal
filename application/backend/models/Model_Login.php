		...
		if (empty($row['id'])) {
			...
			switch ($row['role']) {
				case $user::ROLE_GUEST:
					$form['error_msg'] = 'Гости не могут пользоваться интерфейсом администратора!';
					$user->unsetUser();
					break;
				case $user::ROLE_MANAGER:
					$form['error_msg'] = 'Модераторы не могут пользоваться интерфейсом администратора!';
					$user->unsetUser();
					break;
				case $user::ROLE_ADMIN:
					if (!$user->checkHash($form['pwd'], $row['pwd_hash'])) {
						// invalid password
					...
					break;
				default:
					$form['error_msg'] = 'Учетная запись с неизвестной ролью!';
					$user->unsetUser();
			}
		}
		return $form;
