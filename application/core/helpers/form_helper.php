...
define('PATTERN_PHONE_ADD', '/^[\+0-9,;\s]*$/u');
define('MSG_PHONE_ADD', 'Знак "+", цифры, запятые, точки с запятой и пробелы');
        ...
        if (isset($rules) && is_array($rules)) {
            ...
            return '<div class="form-group row" id="' . $rules['control'] . '_div">' .
                ...
                HTML_Helper::setInput($rules['type'],
                    ...
                    $rules['value'],
                    (isset($rules['events'])) ? $rules['events'] : NULL,
                    (isset($rules['disabled'])) ? $rules['disabled'] : NULL) .
    ...
    */
    public static function setFormRadio($rules): string
    {
        if (isset($rules) && is_array($rules)) {
            ...
            if(isset($rules['disabled']) && $rules['disabled'] === TRUE) {
                $disabled = ' readonly ';
            } else {
                $disabled = '';
            }
    ...
    public static function setFormCheckbox($rules): string
    {
        if (isset($rules) && is_array($rules)) {
            ...
            return '<div class="form-check">
						<div class="col">
							<input '.$disabled.' type="checkbox" class="' . $rules['class'] . '" id="' . $rules['control'] . '" name="' . $rules['control'] . '" ' . $rules['value'] . '><b>' . $rules['label']
                . '</b>' .
                HTML_Helper::setValidFeedback($rules['error'], $rules['success']) .
                HTML_Helper::setInvalidFeedback($rules['error']) .
                '</div>
    ...
    public static function setFormSelectListDB($rules): string
    {
        if (isset($rules) && is_array($rules)) {
            ...
            $result = '<div class="form-group row" id="' . $rules['control'] . '_div">' .
                ...
                '</div>' .
                '<div class="col">' .
                '<select '.$disabled.' class="' . $rules['class'] . '" id="' . $rules['control'] . '" name="' . $rules['control'] . '">';
            if (isset($rules['model_class']) && !empty($rules['model_class']) && isset($rules['model_method']) && !empty($rules['model_method'])) {
                ...
                foreach ($table as $caption => $row) {
                    if (isset($rules['grouped']) && $rules['grouped']) {
                        $result .= '<optgroup label="' . $caption . '">';
                        foreach ($row as $record) {
                            $result .= '<option value="' . $record[$rules['model_field']] . '"' .
                                (($rules['value'] === $record[$rules['model_field']]) ? ' selected' : '') . '>' .
                                ((isset($rules['model_field_name'])) ? $record[$rules['model_field_name']] : $record[$rules['model_field']]) .
                                '</option>';
                        }
                        $result .= '</optgroup>';
                    } else {
        ...
        if (isset($rules) && is_array($rules)) {
            ...
            if (isset($rules['data'][$field . '_id'])) {
                $result .= '<input type="hidden" id="' . $field . '_id" name="' . $field . '_id" value="' . $rules['data'][$field . '_id'] . '"/>' .
                    '<span style="padding-left:10px;"> </span><img class="img-fluid" src="data:' . $rules['data'][$field . '_type'] . ';base64,'
                    . base64_encode($rules['data'][$field]) . '" width="80" height="100">' .
                    '<span style="padding-left:10px;"> </span>' .
                    HTML_Helper::setHrefButtonIcon('Scans', 'Show/?id=' . $rules['data'][$field . '_id'] . ((isset($rules['home_id']) && !empty($rules['home_id'])) ? '&pid='
                            . $rules['home_id']
                            : '') . '&hdr=' . $rules['home_hdr'] . '&ctr=' . $rules['home_ctr'] . '&act=' . $rules['home_act'], 'font-weight-bold',
                        'far fa-file-image fa-2x', 'Просмотреть файл') .
                    '<span style="padding-left:10px;"> </span>' .
                    HTML_Helper::setHrefButtonIcon('Scans',
                        'DeleteConfirm/?id=' . $rules['data'][$field . '_id'] . ((isset($rules['home_id']) && !empty($rules['home_id'])) ? '&pid='
                            . $rules['home_id']
                            : '') . '&hdr=' . $rules['home_hdr'] . '&ctr=' . $rules['home_ctr'] . '&act=' . $rules['home_act'],
                        'text-danger font-weight-bold', 'fas fa-times fa-2x', 'Удалить файл');
            } else {
    ...
    public static function setFormFileListDB($rules): string
    {
        if (isset($rules) && is_array($rules)) {
            ...
            $result .= '<div class="col">' .
                ...
                '<p class="font-weight-bold font-italic">Документы, отмеченные звездочкой (<span style="color: #ff0000;">*</span>), обязательны для загрузки</p>' .
                '</div>';
            // set help
            $result .= '<div class="col">' .
                '<div class="alert alert-info">'
            ...
            foreach ($table as $row) {
                ...
                if (isset($rules['required'])) {
                    if ($row[$rules['required']] == 1) {
                        $label = self::setFormLabelStyle('yes', $rules['required_style'] ?? NULL, $row[$rules['model_field_name']]);
                    } else {
                        $label = self::setFormLabelStyle('no', $rules['required_style'] ?? NULL, $row[$rules['model_field_name']]);
                    }
                ...
                // next input
                if (isset($rules['data'][$field . '_id']) && $rules['model_filter_var'] != 'application_recall') {
                    // file exists
                    $result .= '<input type="hidden" id="' . $field . '_id" name="' . $field . '_id" value="' . $rules['data'][$field . '_id'] . '"/>' .
                        '<span style="padding-left:10px;"> </span><img class="img-fluid" src="data:' . $rules['data'][$field . '_type'] . ';base64,'
                        . base64_encode($rules['data'][$field]) . '" width="80" height="100">' .
                        '<span style="padding-left:10px;"> </span>' .
                        HTML_Helper::setHrefButtonIcon('Scans',
                            'Show/?id=' . $rules['data'][$field . '_id'] . ((isset($rules['home_id']) && !empty($rules['home_id'])) ? '&pid='
                                . $rules['home_id']
                                : '') . '&ctr=' . $rules['home_ctr'] . '&act=' . $rules['home_act'], 'font-weight-bold', 'far fa-file-image fa-2x',
                            'Посмотреть файл') .
                        '<span style="padding-left:10px;"> </span>';
                    if (!isset($rules['editable']) || $rules['editable'] !== FALSE) {
                        $result .= HTML_Helper::setHrefButtonIcon('Scans',
                                'DeleteConfirm/?id=' . $rules['data'][$field . '_id'] . ((isset($rules['home_id']) && !empty($rules['home_id']))
                                    ? '&pid=' . $rules['home_id'] : '') . '&hdr=' . $rules['home_hdr'] . '&ctr=' . $rules['home_ctr'] . '&act='
                                . $rules['home_act'], 'text-danger font-weight-bold', 'fas fa-times fa-2x', 'Удалить файл') . '</div>';
                    }
                } else {
                    // file not exists
        ...
        if ($required === 'yes') {
            switch ($required_style) {
                case 'StarUp':
                    ...
                    $label['value'] = $value . ' <span style="vertical-align: text-top; color: #ff0000">*</span>';
                    break;
                case 'RedBold':
