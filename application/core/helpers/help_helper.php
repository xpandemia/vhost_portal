...
//				</p>';
		//вмешательство паши решение И.А.
		return '<p>
					<strong>Уважаемый абитуриент!</strong><br><br>
					Когда Вы видите свою анкету впервые, она пуста и имеет состояние <strong>"'.$resume::STATUS_CREATED_NAME.'"</strong>. Это означает, что она готова для заполнения. Пожалуйста, аккуратно заполните нужные поля <strong>(* - означает, что поле обязательно для заполнения)</strong> и нажмите кнопку <strong>"Сохранить"</strong>. После этого система проверит введенные Вами данные и<br>
					<u>- если ошибок нет</u>, сохранит их для дальнейшего использования, анкета получит состояние <strong>"'.$resume::STATUS_SAVED_NAME.'"</strong> и Вы увидите выделенное зелёным цветом сообщение "Анкета сохранена."<br>
					<u>- если ошибки есть</u>, они отобразятся красным цветом, а состояние анкеты не изменится. Исправьте ошибки и снова нажмите <strong>"Сохранить"</strong>. Повторяйте до тех пор, пока не увидите выделенное зелёным цветом сообщение "Анкета сохранена."<br><br>
					Анкету в состоянии <strong>"'.$resume::STATUS_SAVED_NAME.'"</strong> можно исправлять без ограничений, но на её основании нельзя формировать заявления на поступление. Чтобы получить такую возможность, необходимо перевести анкету в состояние <strong>"'.$resume::STATUS_SENDED_NAME.'"</strong>. Для этого нажмите кнопку <strong>"Отправить"</strong>, когда Вами проверены все внесённые данные и Вы будете готовы двигаться дальше.<br><br>
					Анкеты в состоянии <strong>"'.$resume::STATUS_SENDED_NAME.'"</strong> ИСПРАВЛЯТЬ НЕЛЬЗЯ.
				</p>';
	}
	...
	public static function app_help() : string
	{
		...
		return '<p>
					...
					- для <u>принятых</u> заявлений:<br>
					Откройте заявление, нажав на <i class="far fa-edit"></i>, и нажмите кнопку <strong>"Изменить"</strong>. После внесения необходимых изменений, распечатайте заявление, подпишите, отсканируйте/сфотографируйте, загрузите его в специально предназначенные для этого поля, сохраните и отправьте в приёмную комиссию.<br>
					- для <u>отклонённых</u> заявлений:<br>
					Просто создайте новое заявление.<br><br>
